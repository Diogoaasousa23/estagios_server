...
class ESTG : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val isFavorito = sharedPreferences.getBoolean("ESTG_Favorito", false)
        ...
        checkboxFavorito.setOnCheckedChangeListener { _, isChecked ->
            ...
            editor.putBoolean("ESTG_Favorito", isChecked)
            editor.apply()
        }
        ...
        call.enqueue(object : Callback<List<Escola>> {
            override fun onResponse(call: Call<List<Escola>>, response: Response<List<Escola>>) {
                if (response.isSuccessful) {
                    val cursos = response.body()?.find { it.escola == "ESTG" }?.cursos ?: emptyList()
